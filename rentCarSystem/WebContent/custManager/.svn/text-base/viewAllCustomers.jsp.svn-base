...
function checkDel(identity){
	if(confirm("您确定要删除吗？")){
		document.forms[0].action="deleteCustomersServlet.do";
...
function checkShow(identity){
	location.href='findCustomersServlet.do?identity=' + identity ;
}
<!-- 复选框批量删除js-->
	function selectCar(obj){
	<!-- 获取所有的复选框-->
			var cust = document.getElementsByName("custId");
			<!-- 设置全选-->
			if(obj==0){
...
function qxSelect(obj){
		...
		function deleteAll(){
			...
			if(flag){
				if(confirm("删除后不能恢复，确认删除？")){
					document.forms[0].action="deleteAllCustomersServlet.do";
					document.forms[0].submit();
				}
			}else{
				alert("至少选择一个客户");
	...
	<body>
		<form action="findAllCustomersServlet.do" method="post">
												...
												<table width="100%" border="0" cellspacing="0"
													...
													<tr>
														...
														<td width="94%" valign="middle">
															<span class="STYLE1" style="color: white;">&nbsp;&nbsp;客户基本信息列表</span>
														</td>
													</tr>
												</table>
											</td>
					...
					<td>
						<table width="100%" border="0" cellpadding="0" cellspacing="1"
							...
							<input type="hidden" name="identity"
								value="${requestScope.customers.identity }" />
							<input type="hidden" name="custName"
								value="${requestScope.customers.custName }" />
							<input type="hidden" name="sex"
								value="${requestScope.customers.sex }" />
							<input type="hidden" name="address"
								value="${requestScope.customers.address }" />
							<input type="hidden" name="phone"
								value="${requestScope.customers.phone }" />
							<input type="hidden" name="career"
								value="${requestScope.customers.career }" />
							...
							<c:forEach var="u" items="${requestScope.page.result }">
								<tr>
									<td height="20" bgcolor="#FFFFFF">
										<div align="center">
											<input type="checkbox" name="custid" id="custId"
												value="${u.identity}" />
									...
									<td height="20" bgcolor="#FFFFFF" class="STYLE6">
										<div align="center">
											<span class="STYLE19">${u.identity}</span>
										</div>
									</td>
